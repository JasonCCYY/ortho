...
function fetchClinicHtml(dateStr) {
  const path = dateStr
  ...
  return new Promise((resolve, reject) => {
    const req = http.request({
      ...
      res.on('end', () => {
        ...
        if (charset !== 'utf-8' && charset !== 'utf8') html = iconv.decode(buf, charset);
        resolve(html);
      });
    });
...
function parseDoctors(html) {
  const results = [];
  const shiftMap = { '早上': { label: '早上', cls: 'tag-am' }, '下午': { label: '下午', cls: 'tag-pm' }, '晚上': { label: '夜診', cls: 'tag-night' } };
  for (const doctor of DOCTORS) {
    ...
    while ((match = linkRegex.exec(html)) !== null) {
      ...
      results.push({ dateKey: `${yyyy}${mm}${dd}`, date: `${mm}/${dd}(${dow})`, doctor, session: shiftInfo.label, cls: shiftInfo.cls, room, num: visitno, numCls: '' });
    }
  }
  return results;
}
...
module.exports = async (req, res) => {
  try {
    ...
    const [h1, h2] = await Promise.all([fetchClinicHtml(null), fetchClinicHtml(fmtDate(nextWeek))]);
    let results = [...parseDoctors(h1), ...parseDoctors(h2)];

    const seen = new Set();
    results = results.filter(r => { const k = r.dateKey + r.doctor + r.session; if (seen.has(k)) return false; seen.add(k); return true; });
